...
public class OrderDetailService {
    ...
    public List<OrderDetailCustomer> getAllOrderDetailsWithProductDetails() {
        String query = "SELECT "
                + "    o.order_id, "
                + "    o.orderDate, "
                + "    o.order_status, "
        ...
        try {
            List<OrderDetailCustomer> orderDetailsList = new ArrayList<>();
            connection = dbcontext.getConnection();
            ps = connection.prepareStatement(query);
            rs = ps.executeQuery();
            while (rs.next()) {
                ...
                orderDetailsList.add(orderDetailCustomer);
            }
    ...
    public List<OrderDetailCustomer> getAllOrderDetailsWithProductDetailsHaveFB() {
        String query = "SELECT "
                + "    o.order_id, "
                + "    o.orderDate, "
                + "    o.order_status, "
                + "    o.shipping_status, "
                + "    od.order_detail_id, " // Thêm cột order_detail_id
                + "    p.pro_name, "
                + "    od.UnitPrice, "
                + "    od.Quantity, "
                + "    pc.color_name, "
                ...
                + "JOIN ProductSizes ps ON pv.size_id = ps.size_id "
                + "JOIN ProductImages pi ON pv.image_id = pi.image_id "
                + "WHERE od.feedback_details IS NOT NULL "
                + "ORDER BY o.order_id";
    ...
    public List<OrderDetailCustomer> searchOrderDetailsByProductName(String proName) {
        String query = "SELECT "
                + "    o.order_id, "
                + "    o.orderDate, "
                + "    o.order_status, "
    ...
    public void deleteFeedbackOrderDetail(int orderdetail_id) {
        String query = "Update OrderDetails set feedback_details = NULL WHERE order_detail_id = ?";
        try {
            connection = dbcontext.getConnection();
            ps = connection.prepareStatement(query);
    ...
    public List<OrderDetailCustomer> searchOrderDetailsCustomer(String orderId, String nameProduct, String priceFromStr, String priceToStr, String orderDateFrom, String orderDateTo, String size, String color) {
        String baseQuery = "SELECT o.order_id, o.orderDate, p.pro_name, od.UnitPrice, od.Quantity, pc.color_name, ps.size_name, oi.imageURL, o.username, o.acc_id, o.addressReceive, o.phone, od.feedback_details, o.feedback_order, o.order_status, o.pay_status, o.shipping_status, od.order_detail_id "
                + "FROM Orders o "
                + "JOIN OrderDetails od ON o.order_id = od.order_id "
                + "JOIN ProductVariants pv ON od.variant_id = pv.variant_id "
                + "JOIN Products p ON pv.pro_id = p.pro_id "
        ...
        try {
            List<OrderDetailCustomer> ls = new ArrayList<>();
            connection = dbcontext.getConnection();
            ps = connection.prepareStatement(baseQuery);
            int paramIndex = 1;
            ps.setString(paramIndex++, orderId);
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                ls.add(new OrderDetailCustomer(
                        rs.getInt("order_id"),
                        rs.getDate("orderDate"),
                        OrderStatus.values()[rs.getInt("order_status")],
                        ShipStatus.values()[rs.getInt("shipping_status")],
                        rs.getInt("order_detail_id"), // Include order_detail_id here
                        rs.getString("pro_name"),
                        rs.getDouble("UnitPrice"),
                        rs.getInt("Quantity"),
                        rs.getString("color_name"),
